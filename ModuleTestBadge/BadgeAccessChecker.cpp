...
BadgeAccessChecker::BadgeAccessChecker()
    : m_start(8, 0),    // 08:00 par defaut
      m_end(18, 0)      // 18:00 par defaut
{
}
...
AccessResult BadgeAccessChecker::check(const QString &uid) const
{
    ...
    r.inScheduleSlot = isWithinSchedule(QTime::currentTime());

    // 3. Decision
    r.granted = r.badgeKnown && r.inScheduleSlot;

    if (!r.badgeKnown) {
        r.reason = "Badge inconnu (pas dans la liste blanche)";
