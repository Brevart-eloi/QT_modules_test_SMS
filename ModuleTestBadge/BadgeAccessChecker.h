 ...
 *
 *  La verification est PUREMENT LOCALE (aucune dependance reseau) :
 *    1. Le badge fait-il partie de la liste blanche ? (UID connu)
 *    2. L'heure actuelle est-elle dans la plage horaire autorisee ?
 *
 *  Acces accorde uniquement si les DEUX conditions sont vraies.
 *
 *  Les UID sont normalises (hex minuscule, sans prefixe "0x") pour coller
 *  au format envoye par l'Arduino : String(reversedCode, HEX).
 *
 * ============================================================================
 */
...
public:
    ...
    // ===== Verification =====
    AccessResult check(const QString &uid) const;
